"""
Migration script: savingstracker_backup (prod schema) --> savingstracker_v2_testdev
=======================================================================
...
This script performs a FULL refresh each run:
    A) wipes all destination data in savingstracker_v2_testdev
  ...
  DEST   (write):     [savingstracker_v2_testdev].dbo.*
...
-----
    ...
    python migration/migrate_prod_to_v2_testdev.py --dry-run

    # Live full-refresh migration:
    python migration/migrate_prod_to_v2_testdev.py
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(
            os.path.join(_SCRIPT_DIR, "migration_testdev.log"), mode="a", encoding="utf-8"
        ),
    ],
)
...
def get_connections(src_db_override=None):
    ...
    dest_db = os.getenv("DB_NAME_TESTDEV", "savingstracker_v2_testdev")
...
def reset_destination_db(dest, dry_run):
    """
    Wipe all v2 testdev data and reset identities to 1 for all migrated tables,
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Migrate savingstracker_backup -> savingstracker_v2_testdev"
    )
...
def main():
    ...
    log.info("=" * 60)
    log.info("savingstracker_backup  -->  savingstracker_v2_testdev  MIGRATION")
